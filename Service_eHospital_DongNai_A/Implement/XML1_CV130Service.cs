...
namespace Service_eHospital_DongNai_A.Implement
{
    ...
    , IXML1_CV130Service
    {
        ...
        public virtual async Task<List<XML1_CV130>> GetByYear_Month_Day_SearchStringToListAsync(int Year, int Month, int Day, string SearchString)
        {
            List<XML1_CV130> result = new List<XML1_CV130>();
            if (!string.IsNullOrEmpty(SearchString))
            {
            ...
            else
            {
                ...
                if (Day > 0)
                {
                    if (Day > 9)
                    {
                        SearchString = SearchString + Day.ToString();
                    }
                    else
                    {
                        SearchString = SearchString + "0" + Day.ToString();
                    }
                }
