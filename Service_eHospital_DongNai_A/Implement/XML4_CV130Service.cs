        ...
        public override async Task<List<XML4_CV130>> GetBySearchStringToListAsync(string searchString)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(searchString))
                {
                    ...
                    if (result.Count == GlobalHelper.InitializationNumber)
                    {
                        result = await GetByCondition(item => item.MA_DICH_VU.Trim() == searchString).ToListAsync();
                    }
                    if (result.Count == GlobalHelper.InitializationNumber)
                    {
                        result = await GetByCondition(item => item.MA_BS_DOC_KQ.Trim() == searchString).ToListAsync();
                    }
                }
            }
            catch (Exception ex)
