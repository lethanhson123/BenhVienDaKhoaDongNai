...
@Component({
  selector: 'app-phong-kham-id',
  templateUrl: './phong-kham-id.component.html',
  styleUrls: ['./phong-kham-id.component.css']
})
export class PhongKhamIDComponent implements OnInit {
  ...
  DanhMucPhongKhamSearch() {
    if (this.DanhMucPhongKhamService.BaseParameter.SearchString.length > 0) {
      ...
      this.DanhMucPhongKhamService.BaseParameter.SearchString = this.ActiveRouter.snapshot.params.SearchString;
      this.DanhMucPhongKhamService.GetSQLBySearchStringViaIDToListAsync().subscribe(
        res => {
  ...
  DanhMucPhongKhamActiveChange(element: DanhMucPhongKham) {
    if (element.Active == true) {
      ...
      console.log(this.DanhMucPhongKhamService.List);
    }
  }
  ...
  NgayCapSoSoThuTuStringKeyPress(event: KeyboardEvent) {
    if (event.keyCode == 13) {
      this.GoiSoChiTietService.BaseParameter.DanhMucDichVuID = this.GoiSoChiTietService.FormData.DanhMucDichVuID;
      this.GoiSoChiTietService.BaseParameter.NgayCapSoSoThuTu = this.GoiSoChiTietService.FormData.NgayCapSoSoThuTu;
