namespace API.Controllers.v1
{
    ...
    public class GoiSoChiTietController : BaseController<GoiSoChiTiet, IGoiSoChiTietService>
    {
        ...
        [HttpPost]
        [Route("GetByKhachHangIDToListAsync")]
        public virtual async Task<List<GoiSoChiTiet>> GetByKhachHangIDToListAsync()
        {
            List<GoiSoChiTiet> result = new List<GoiSoChiTiet>();
            try
            {
                BaseParameter baseParameter = JsonConvert.DeserializeObject<BaseParameter>(Request.Form["data"]);
                result = await _GoiSoChiTietService.GetByKhachHangIDToListAsync(baseParameter.KhachHangID.Value);
            }
            catch (Exception ex)
            {
                string message = ex.Message;
