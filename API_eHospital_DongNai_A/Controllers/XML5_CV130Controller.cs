namespace API_eHospital_DongNai_A.Controllers.v1
{
    ...
    public class XML5_CV130Controller : BaseController<XML5_CV130, IXML5_CV130Service>
    {
        ...
        public virtual async Task<List<XML5_CV130>> GetByListIDStringToListAsync()
        {
            List<XML5_CV130> result = new List<XML5_CV130>();
            try
            {
                BaseParameter baseParameter = JsonConvert.DeserializeObject<BaseParameter>(Request.Form["data"]);
                result = await _XML5_CV130Service.GetByListIDStringToListAsync(baseParameter.ListIDString);
            }
            catch (Exception ex)
            {
                string message = ex.Message;
