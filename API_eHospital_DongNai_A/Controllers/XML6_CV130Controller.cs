namespace API_eHospital_DongNai_A.Controllers.v1
{
    ...
    public class XML6_CV130Controller : BaseController<XML6_CV130, IXML6_CV130Service>
    {
        ...
        public virtual async Task<List<XML6_CV130>> GetByListIDStringToListAsync()
        {
            List<XML6_CV130> result = new List<XML6_CV130>();
            try
            {
                BaseParameter baseParameter = JsonConvert.DeserializeObject<BaseParameter>(Request.Form["data"]);
                result = await _XML6_CV130Service.GetByListIDStringToListAsync(baseParameter.ListIDString);
            }
            catch (Exception ex)
            {
                string message = ex.Message;
